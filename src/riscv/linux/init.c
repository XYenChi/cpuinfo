...
static uint32_t cpuinfo_riscv_find_cache_index(uint32_t cpu_id, uint32_t level, const char* type) {
	...
	for (uint32_t idx = 0; idx < CPUINFO_RISCV_CACHE_MAX_INDICES; idx++) {
		...
		snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%u/cache/index%u/level", cpu_id, idx);
		...
		/* Check the type. */
		snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%u/cache/index%u/type", cpu_id, idx);
...
static bool cpuinfo_riscv_read_cache_from_sysfs(
	...
	snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%u/cache/index%u/size", cpu_id, cache_index);
	uint32_t size = 0;
	...
	snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%u/cache/index%u/number_of_sets", cpu_id, cache_index);
	uint32_t sets = 0;
	...
	snprintf(
		path,
		sizeof(path),
		"/sys/devices/system/cpu/cpu%u/cache/index%u/coherency_line_size",
		cpu_id,
		cache_index);
	...
	snprintf(
		path,
		sizeof(path),
		"/sys/devices/system/cpu/cpu%u/cache/index%u/physical_line_partition",
		cpu_id,
		cache_index);
...
void cpuinfo_riscv_linux_init(void) {
	...
	for (size_t processor = 0; processor < max_processor_id; processor++) {
		...
		/* Populate L1I and L1D caches for this logical processor from sysfs. */
		{
			...
			if (l1i_idx != UINT32_MAX && cpuinfo_riscv_read_cache_from_sysfs(linux_id, l1i_idx, &l1i[pi])) {
			...
			if (l1d_idx != UINT32_MAX && cpuinfo_riscv_read_cache_from_sysfs(linux_id, l1d_idx, &l1d[pi])) {
			...
			/* Populate L2 cache for this core from sysfs. */
			{
				...
				if (l2_idx != UINT32_MAX) {
					...
					if (shared_l2_index == UINT32_MAX &&
					    cpuinfo_riscv_read_cache_from_sysfs(
						    linux_id, l2_idx, &l2[valid_l2_count])) {
		...
		if (core_to_l2_index[valid_cores_index - 1] != UINT32_MAX) {
			processors[valid_processors_index - 1].cache.l2 = &l2[core_to_l2_index[valid_cores_index - 1]];
		}
